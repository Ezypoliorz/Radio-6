...
def AjouterEmission(titre, date, audio, programme) :
    ...
    html_ajout = f"""
        ...
        <div class="div-programme-background" id="Programme-background5">
    ...
    for i in range(len(programme)) :
        ...
        if programme[0].split(" - ")[i].split(":")[0] == "0" :
            minute_programme_réel = int(programme[i].split(" - ")[0].split(":")[0][:-1])*60
        else :
            minute_programme_réel = int(programme[i].split(" - ")[0].split(":")[0])*60

        if programme[0].split(" - ")[i].split(":")[0] == "0" :
            seconde_programme_réel = int(programme[i].split(" - ")[1].split(":")[1][:-1])
        else :
            seconde_programme_réel = int(programme[i].split(" - ")[1].split(":")[1])
